...
- [よけとる設計練習](https://am1tanaka.github.io/YoketoruPracticeDocs/index.html)
  - 準備
    - 指定の座席へ移動
    - GitHubを開いて、サインイン
    - 「第3章 プロジェクトの準備」から、フォークして、クローンするまで確認
      - フォークしているはずだが、フォークしていなければ、ここでフォークする
    - 移動先の席で、フォークしたリポジトリーを、クローンして、Unityで開く
  - 「第5章　プレイヤーの開発」を読んで、Playerの開発に挑戦する
  - 少し作業して、コミットとプッシュして、プルリクエストを作成する
  - 時間になったら、コミットとプッシュする
...
- 15発目の弾の消し方
  - 変数を増やす必要はありませんでした。終了判定を、弾が消える場所に移動させればOK
...
- 展示のまとめ
  ...
  - 共有をクリックして、リンクを知っている全員が編集できるように設定して、共有リンクを[こちら](https://docs.google.com/forms/d/e/1FAIpQLSdtCrSbaVfK9kttxBd6T577bJhaQ1RXeWdHg7h03dKhlvEm9g/viewform?usp=sf_link)に報告して、[Slack](https://datgm24.slack.com)の自分のチャンネルに貼り付ける
...
### 資料
- [チーム開発-Unityパッケージを利用した簡単なUnityチーム開発](https://docs.google.com/document/d/10UpsXhB-dGMrBqmMvZow-xGcs57YMysIfNekGgEWkzM/)
...
- DATフェスタ企画の検討
  - 以下の企画をベースに検討を進める
    ...
    - 真夜中の教室
      - スサ、ナカヤマ
  - [SPACE SHOOTER作業メモ](https://docs.google.com/document/d/1cbr9tilZ6O1Jwr_D0N4AhZcrBAdRBKXqqSGH5Gb8WDY/) 敵A, B, Cの検討から
  - [水中探索作業メモ](https://docs.google.com/document/d/1P0pPZxh-_32VVJR4lYr1o7OXSe--rCjcRLYNqejPSeQ/)
  - 企画のブラッシュアップの方針の検討
...
1. Visual C#の新規プロジェクトを作成して、名前を`fukuv0614`にする
1. GitHubにPublishする
...
3. int型のインスタンス変数`ans`を定義して、`0`～`50`の乱数を代入する
...
5. ボタンが押された処理として、以下を実装する
   ...
   5. ansがyosouと等しかったら、MessageBoxで`Bingo!!`と表示して、countに0を代入して、ansに0～50までの乱数を求め直す
   6. ansがyosouより小さかったら、MessageBoxで`Less`と表示
   7. ansがyosouより大きかったら、MessageBoxで`Greater`と表示
